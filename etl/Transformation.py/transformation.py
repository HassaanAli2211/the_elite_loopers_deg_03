...
lux_meter= lux_meter.withColumn("message_content", F.from_json(F.col("value").cast("string"),lux_meter_schema))
...
lux_meter = lux_meter.selectExpr("CAST(value AS STRING)")
lux_meter = lux_meter.select(from_json(lux_meter.value, lux_meter_schema).alias("data"))
...
lux_meter_df = lux_meter.select("room_id", F.col("measurements.timestamp").alias("timestamp"), F.col("measurements.light_level").alias("light_level"))
...
joined_df = joined_df.select("date", "Temperature","humidity", "light_level" ,"co2", "humidity_ratio", "room_id")
...
df_to_kafka_streaming = joined_df.withColumn("data_for_kafka", F.to_json(F.struct(*joined_df.columns))) \
                                        .select(F.col("data_for_kafka").alias("value"))

df_to_kafka_streaming_query = df_to_kafka_streaming.writeStream \
  .format("kafka") \
