...
moisture_mate_reader= spark.readStream \
                          ...
                          .option("startingOffsets", "latest")
smart_thermo_reader= spark.readStream \
                          .format("kafka") \
                          .option("kafka.bootstrap.servers", "kafka:29092") \
...
smart_thermo= smart_thermo_df.withColumn("message_content", F.from_json(F.col("value").cast("string"),smart_thermo_schema))
...
smart_thermo = smart_thermo.select(F.from_json(smart_thermo.value, smart_thermo_schema).alias("data"))
